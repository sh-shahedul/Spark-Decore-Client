...
const EarningSummary = () => {
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6">
      ...
      {/* Earnings Summary */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <div className="p-3 sm:p-4 md:p-6 bg-green-100 dark:bg-green-900/50 rounded shadow text-center">
          <h3 className="font-semibold text-sm sm:text-base md:text-lg whitespace-nowrap dark:text-gray-200">Total Earnings (BDT)</h3>
          <p className="text-xl sm:text-2xl md:text-3xl font-bold dark:text-green-300">{totalEarnings || 0}</p>
        </div>
        <div className="p-3 sm:p-4 md:p-6 bg-blue-100 dark:bg-blue-900/50 rounded shadow text-center">
          <h3 className="font-semibold text-sm sm:text-base md:text-lg whitespace-nowrap dark:text-gray-200">Today's Earnings (BDT)</h3>
          <p className="text-xl sm:text-2xl md:text-3xl font-bold dark:text-blue-300">{todayEarnings || 0}</p>
        </div>
        <div className="p-3 sm:p-4 md:p-6 bg-yellow-100 dark:bg-yellow-900/50 rounded shadow text-center">
          <h3 className="font-semibold text-sm sm:text-base md:text-lg whitespace-nowrap dark:text-gray-200">Completed Projects</h3>
          <p className="text-xl sm:text-2xl md:text-3xl font-bold dark:text-yellow-300">{totalCompletedProjects || 0}</p>
        </div>
      </div>

      {/* Booking Details Table */}
      <div className="overflow-x-auto shadow-lg rounded-lg bg-white dark:bg-slate-800">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
          <thead className="bg-[#007b91] dark:bg-slate-900 text-white">
            <tr>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">User Name</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap"> Service</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">Email</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
            {bookings?.map((b) => (
              <tr key={b._id} className="hover:bg-gray-50 dark:hover:bg-slate-700 transition">
                <td className="px-2 sm:px-3 py-2 text-xs sm:text-sm text-sky-600 dark:text-sky-400 font-semibold whitespace-nowrap">
                  <span className='bg-sky-100 dark:bg-sky-900/50 px-2 py-1 rounded-full'>{b.userName}</span>
                </td>
                <td className="text-xs sm:text-sm text-[#FAB12F] dark:text-[#FFD700] font-bold whitespace-nowrap">{b.serviceName}</td>
                <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                  <span className="bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-300 px-2 py-1 rounded-full font-semibold text-xs sm:text-sm">
                    {b.userEmail}
                  </span>
                </td>
                <td className="px-2 sm:px-3 py-2 text-xs sm:text-sm font-bold text-green-600 dark:text-green-400 whitespace-nowrap"> {b.totalCost}</td>
                <td className="px-2 sm:px-3 py-2 text-xs sm:text-sm text-gray-600 dark:text-gray-300 font-bold whitespace-nowrap">{b.bookingDate}</td>
                <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                  <span className={`badge badge-sm text-xs ${b.paymentStatus === 'paid' ? 'badge-success dark:bg-green-700 dark:border-green-600 font-medium' : 'badge-warning dark:bg-yellow-700 dark:border-yellow-600'}`}>
                    {b.assignedDecoatorStatus}
                  </span>
                </td>
              </tr>
