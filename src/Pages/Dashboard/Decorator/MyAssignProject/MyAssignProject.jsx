...
const MyAssignProject = () => {
  ...
  return (
    <div className="sm:p-6">
      {/* Header */}
       <title>spark decore | My Assign Project</title>
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#005461] dark:text-[#4DD4E8] text-center p-8">
          My Assigned <span className="text-[#FAB12F] dark:text-[#FFD700]">Projects</span>
        </h2>
     

      {/* Stats Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-10 bg-[#005461] dark:bg-slate-800 p-8 rounded-xl text-center">
          <div className="text-gray-500 dark:text-gray-300 font-bold bg-white dark:bg-slate-700 px-2 py-3 rounded-xl sm:text-xl">
            Total Assigned : <span className='text-sky-500 dark:text-sky-400'>{totalProjects}</span>
          </div>
          <div className="text-gray-500 dark:text-gray-300 font-bold bg-white dark:bg-slate-700 px-2 py-3 rounded-xl sm:text-xl">
            Paid Projects : <span className='text-[#FAB12F] dark:text-[#FFD700]'>{paidProjects}</span>
          </div>
          <div className="text-gray-500 dark:text-gray-300 font-bold bg-white dark:bg-slate-700 px-2 py-3 rounded-xl sm:text-xl flex items-center justify-center gap-1">
            Total Revenue : <span className='mt-0.5 flex items-center gap-1 text-green-500 dark:text-green-400'>{totalRevenue} <TbCoinTakaFilled /></span>
          </div>
      </div>
  ...
  <div
    key={project._id}
    className="relative group bg-white dark:bg-slate-800 rounded-2xl shadow-md hover:shadow-xl dark:shadow-slate-900/50 transition-all duration-300 overflow-hidden border border-gray-100 dark:border-slate-700"
  >
    {/* Top gradient color */}
    <div className="h-1 w-full bg-gradient-to-r from-[#005461] via-[#FAB12F] to-accent dark:from-[#4DD4E8] dark:via-[#FFD700] dark:to-purple-500"></div>

    <div className="p-4 sm:p-5 md:p-6 flex flex-col justify-between h-full">
      {/* Header */}
      <div>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3">
          <h3 className="text-lg sm:text-xl md:text-2xl font-semibold line-clamp-2 text-gray-800 dark:text-white break-words">
            {project.serviceName}
          </h3>
          <span className="mt-2 sm:mt-0 bg-[#005461] dark:bg-slate-700 text-white font-semibold px-3 py-1 rounded-full text-xs sm:text-sm">
        ...
        <div className="flex items-center gap-3 mb-3">
          <img className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover" src={project.photoURL} alt={project.userName} />
          <div className="truncate">
            <p className="font-medium text-gray-700 dark:text-gray-200 text-sm sm:text-base truncate">{project.userName}</p>
            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate">{project.userEmail}</p>
          </div>
        </div>

        {/* Project Details */}
        <div className="space-y-1 sm:space-y-2 text-sm sm:text-base text-gray-700 dark:text-gray-300">
          <p className="flex items-center gap-2">
            <FaCalendarAlt className="text-primary dark:text-[#4DD4E8]" />
            <span className="font-bold">Date:</span><span className='text-primary dark:text-[#4DD4E8] font-semibold'> {project.bookingDate}</span>
          </p>
          <p className="flex items-center gap-2">
            <FaMoneyBillWave className="text-green-500 dark:text-green-400" />
            <span className="font-bold flex items-center">Total: &nbsp; <span className='text-green-600 dark:text-green-400 flex items-center font-semibold'>{project.totalCost} <TbCurrencyTaka size={20} /> </span></span>
          </p>
        </div>
      </div>

      {/* Status Badges */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4 pt-4 border-t border-gray-100 dark:border-slate-700 gap-2 sm:gap-0">
        {/* Payment Status */}
        <span
          className={`${
            project.paymentStatus === 'paid'
              ? 'bg-green-200 dark:bg-green-900/50 text-green-600 dark:text-green-300 font-semibold px-3 py-1 rounded-full'
              : 'bg-yellow-200 dark:bg-yellow-900/50 text-yellow-600 dark:text-yellow-300 font-semibold px-3 py-1 rounded-full'
          } text-xs sm:text-sm`}
        >
          {project.paymentStatus}
        </span>
    ...
    {/* Hover Overlay */}
    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-300 pointer-events-none bg-gradient-to-tr from-primary/10 dark:from-[#4DD4E8]/10 via-transparent to-secondary/10 dark:to-purple-500/10 rounded-2xl"></div>
  </div>
))}
