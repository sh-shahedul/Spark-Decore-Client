...
// Map status 
const STATUS_COLORS = {
  assigned: "bg-gray-300 dark:bg-gray-700 text-gray-800 dark:text-gray-200",
  planning: "bg-blue-400 dark:bg-blue-600 text-white",
  "materials-prepared": "bg-yellow-400 dark:bg-yellow-600 text-white",
  "on-the-way": "bg-indigo-500 dark:bg-indigo-700 text-white",
  "setup-in-progress": "bg-orange-500 dark:bg-orange-700 text-white",
...
const UpdateProjectStatusRow = ({ booking, index, refetch }) => {
  ...
  return (
    <tr className="hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
      <title>spark decore | Update Project Status</title>
      <th className="px-2 sm:px-3 py-2 text-xs sm:text-sm whitespace-nowrap dark:text-gray-300">{index + 1}</th>
      <td className="px-2 sm:px-3 py-2 font-medium text-xs sm:text-sm whitespace-nowrap">
        <span className="bg-sky-100 dark:bg-sky-900/50 px-2 py-1 rounded-full text-sky-500 dark:text-sky-300">{booking.serviceName}</span>
      </td>
      <td className="px-2 sm:px-3 py-2 text-xs sm:text-sm whitespace-nowrap text-[#FAB12F] dark:text-[#FFD700] font-bold">{booking.userName}</td>
      <td className="px-2 sm:px-3 py-2 text-xs sm:text-sm whitespace-nowrap">
        <span className="text-xs text-gray-500 dark:text-gray-400 font-bold">{booking.bookingDate} <br />{booking.bookingTime}</span>
      </td>
      <td className="px-2 sm:px-3 py-2 text-xs sm:text-sm whitespace-nowrap">
        <span className="text-purple-600 dark:text-purple-300 bg-purple-100 dark:bg-purple-900/50 px-2 py-1 rounded-full font-semibold">{booking.location}</span>
      </td>
      <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
        {booking.paymentStatus === "paid" ? (
          <span className="badge badge-success badge-sm text-xs dark:bg-green-700 dark:border-green-600">Paid</span>
        ) : (
          <span className="badge badge-warning badge-sm text-xs dark:bg-yellow-700 dark:border-yellow-600">Unpaid</span>
        )}
      </td>
      <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
        <span className={`badge badge-sm capitalize text-xs ${STATUS_COLORS[currentStatus]}`}>
      ...
      <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
        {nextStatus ? (
          <button
            onClick={handleUpdateStatus}
            disabled={loading}
            className="btn btn-xs btn-primary hover:scale-105 transition-transform text-xs whitespace-nowrap dark:bg-blue-700 dark:border-blue-600 dark:hover:bg-blue-600"
          >
            {loading ? "Updating..." : `Move to ${nextStatus.replaceAll("-", " ")}`}
          </button>
        ) : (
          <span className="text-green-600 dark:text-green-400 font-semibold text-xs sm:text-sm whitespace-nowrap">Completed</span>
        )}
      </td>
    </tr>
  );
...
const UpdateProjectStatus = () => {
  ...
  if (!assignDeco.length) {
    return <div className="text-center py-10 text-base sm:text-lg dark:text-gray-300">No assigned projects found.</div>;
  }
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6 lg:p-8">
      ...
      <div className="overflow-x-auto rounded-lg shadow-sm bg-white dark:bg-slate-800">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
          <thead className="bg-[#005461] dark:bg-slate-900 text-white">
            <tr>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">#</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">Service Name</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">User</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">Date & Time</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">Location</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap dark:text-white">Payment</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">Status</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm font-semibold whitespace-nowrap">Action</th>
            </tr>
          </thead>
