...
const AddService = () => {
  ...
  return (
    <div className="min-h-screen bg-[#e6f3f5] dark:bg-gray-900 py-8 px-2 sm:px-6 lg:px-12">
      <div className="max-w-5xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-xl dark:shadow-gray-900/50 p-6 sm:p-8 lg:p-10">
        <title>spark decore | Add Service</title>
        {/* Header */}
        <div className="flex items-center gap-3 mb-8">
          <FaPlusCircle className="text-[#FAB12F] dark:text-amber-400 text-3xl" />
          <h2 className="text-2xl sm:text-3xl font-bold text-[#005461] dark:text-teal-400">
            Add New Service
          </h2>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">
              <FaEnvelope className="text-[#005461] dark:text-teal-400" /> Your Email
            </label>
            <input
              value={user?.email || ""}
              readOnly
              className="w-full bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 px-4 py-3 rounded-lg text-gray-600 dark:text-gray-300 cursor-not-allowed"
            />
          </div>
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">
              <FaTag className="text-[#005461] dark:text-teal-400" /> Service Name
            </label>
            <input
              {...register("service_name", { required: true })}
              placeholder="Outdoor Holud Night Event"
              className="w-full border border-gray-300 dark:border-gray-600 px-4 py-3 rounded-lg
              focus:ring-2 focus:ring-[#005461] dark:focus:ring-teal-400 outline-none dark:bg-gray-700 dark:text-white dark:placeholder-gray-400"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">
                <FaMoneyBillWave className="text-[#005461] dark:text-teal-400" /> Cost
              </label>
              <input
                type="number"
                {...register("cost", { required: true })}
            ...
            <div>
              ...
              <select
                {...register("unit", { required: true })}
                className="w-full border border-gray-300 dark:border-gray-600 px-4 py-3 rounded-lg
                focus:ring-2 focus:ring-[#005461] dark:focus:ring-teal-400 outline-none dark:bg-gray-700 dark:text-white"
              >
                <option value="">Select Unit</option>
                <option value="per event">Per Event</option>
                <option value="per hall">Per Hall</option>
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">
              <FaLayerGroup className="text-[#005461] dark:text-teal-400" /> Category
            </label>
            <select
              {...register("service_category", { required: true })}
              className="w-full border border-gray-300 dark:border-gray-600 px-4 py-3 rounded-lg
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">
              <FaImage className="text-[#005461] dark:text-teal-400" /> Image URL
            </label>
            <input
              {...register("image", { required: true })}
              placeholder="https://example.com/image.jpg"
          ...
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-2
            bg-[#005461] dark:bg-teal-600 hover:bg-[#00414c] dark:hover:bg-teal-700 text-white font-semibold
            py-3 rounded-xl transition"
          >
            <FaPlusCircle /> Add Service
          </button>
