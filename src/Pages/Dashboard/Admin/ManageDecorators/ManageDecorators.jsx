...
//       <title>spark decore | Manage Decorator</title>
...
const ManageDecorators = () => {
  ...
  return (
    <div className="px-2 sm:px-4 lg:px-6">
      <title>spark decore | Manage Decorator</title>
      <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-[#FAB12F] dark:text-amber-400 text-center mt-5">Manage Decorators</h2>

      {/* Summary */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-5 md:gap-10 mb-6 sm:mb-10">
        <div className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 p-4 sm:p-6 md:p-10 flex flex-col sm:flex-row justify-center items-center rounded shadow dark:shadow-gray-900/50">
          <span className="text-base sm:text-xl md:text-2xl font-bold whitespace-nowrap">Total Decorator:</span>
          <span className="font-semibold text-lg sm:text-xl ml-0 sm:ml-2">{totalDecorators}</span>
        </div>
        <div className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 p-4 sm:p-6 md:p-10 flex flex-col sm:flex-row justify-center items-center rounded shadow dark:shadow-gray-900/50">
          <span className="text-base sm:text-xl md:text-2xl font-bold whitespace-nowrap">Active Decorator:</span>
          <span className="font-semibold text-lg sm:text-xl ml-0 sm:ml-2">{activeDecorators}</span>
        </div>
        <div className="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 p-4 sm:p-6 md:p-10 flex flex-col sm:flex-row justify-center items-center rounded shadow dark:shadow-gray-900/50">
          <span className="text-base sm:text-xl md:text-2xl font-bold whitespace-nowrap">Disabled Decorator:</span>
          <span className="font-semibold text-lg sm:text-xl ml-0 sm:ml-2">{disabledDecorators}</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto shadow-lg dark:shadow-gray-900/50 rounded-xl bg-white dark:bg-gray-800">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {currentUsers.map((user) => (
              <tr key={user._id} className="text-center hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                  <img src={user.photoURL} alt={user.displayName} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full mx-auto" />
                </td>
                <td className="px-2 sm:px-3 py-2 font-bold text-gray-500 dark:text-gray-300 text-xs sm:text-sm whitespace-nowrap">{user.displayName}</td>
                <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                  <span className="bg-blue-200 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 font-semibold px-2 py-1 rounded-full text-xs sm:text-sm">
                    {user.email}
                  </span>
                </td>
                <td className="px-2 sm:px-3 py-2 text-green-600 dark:text-green-400 font-bold text-xs sm:text-sm whitespace-nowrap">{user.specialty || "-"}</td>
                <td className="px-2 sm:px-3 py-2 text-gray-500 dark:text-gray-400 font-bold text-xs sm:text-sm whitespace-nowrap">{user.phone || "-"}</td>
                <td className="px-2 sm:px-3 py-2 text-yellow-400 font-bold text-xs sm:text-sm whitespace-nowrap">{user.rating || "-"} ⭐</td>
                <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                  {user.role === "decorator" ? (
                    <div className="flex justify-center items-center gap-1 sm:gap-2">
                      ...
                      <button
                        onClick={() => handleToggleStatus(user._id, user.status)}
                        className="btn btn-xs btn-outline font-semibold text-xs dark:text-gray-300 dark:border-gray-600"
                      >
                        {user.status === "active" ? "Disable" : "Enable"}
                      </button>
                    </div>
                ...
                <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                  {user.role === "admin" ? (
                    <span className="text-purple-600 dark:text-purple-400 bg-purple-200 dark:bg-purple-900/40 px-2 py-1 rounded-full font-bold text-xs sm:text-sm">
                      Admin
                    </span>
                  ) : user.role !== "decorator" ? (
                    <button
                    ...
                    <span className="text-green-600 dark:text-green-400 bg-green-200 dark:bg-green-900/40 px-2 py-1 rounded-full font-bold text-xs sm:text-sm">
                      Decorator
                    </span>
                  )}
                </td>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 mt-6 mb-4 flex-wrap">
          <button
            onClick={() => handlePageChange(currentPage - 1)}
            disabled={currentPage === 1}
            className="px-2 py-1 bg-[#859699] dark:bg-gray-700 hover:bg-[#005461] dark:hover:bg-teal-600 cursor-pointer rounded-lg text-white"
          >
            Previous
          </button>
          ...
          {[...Array(totalPages)].map((_, index) => (
            <button
              ...
              className={`btn btn-sm ${
                currentPage === index + 1
                  ? "bg-[#005461] dark:bg-teal-600 text-white font-semibold"
      ...
      {openModal && (
        <dialog open className="modal">
          <div className="modal-box bg-white dark:bg-gray-800 rounded-xl shadow-xl dark:shadow-gray-900/50 p-6 sm:p-8 w-full max-w-lg mx-4">
            <h3 className="font-bold text-xl sm:text-2xl mb-4 sm:mb-6 text-center text-gray-800 dark:text-gray-100">Assign Decorator</h3>

            <form onSubmit={handleAddDecorator} className="space-y-4">
              <input readOnly value={selectedUser.email} className="input input-bordered w-full bg-gray-100 dark:bg-gray-700 dark:text-white dark:border-gray-600 text-sm" />
              <input readOnly value={selectedUser.displayName} className="input input-bordered w-full bg-gray-100 dark:bg-gray-700 dark:text-white dark:border-gray-600 text-sm" />

              <select className="select select-bordered w-full text-sm dark:bg-gray-700 dark:text-white dark:border-gray-600" onChange={(e) => setSpecialty(e.target.value)} value={specialty}>
                <option value="">Select Specialty</option>
                <option value="Wedding">Wedding</option>
                <option value="Corporate">Corporate</option>
                <option value="Birthday">Birthday</option>
              ...
              <input className="input input-bordered w-full text-sm dark:bg-gray-700 dark:text-white dark:border-gray-600" placeholder="Phone" value={phone} onChange={(e) => setPhone(e.target.value)} />
            ...
            <button className="btn btn-outline btn-sm mt-4 sm:mt-6 w-full dark:text-gray-300 dark:border-gray-600" onClick={() => setOpenModal(false)}>
              Close
            </button>
          </div>
        </dialog>
      ...
      {editModal && (
        <dialog open className="modal">
          <div className="modal-box bg-white dark:bg-gray-800 rounded-xl shadow-xl dark:shadow-gray-900/50 p-6 sm:p-8 w-full max-w-lg mx-4">
            <h3 className="font-bold text-xl sm:text-2xl mb-4 sm:mb-6 text-center text-gray-800 dark:text-gray-100">Edit Decorator</h3>

            <form onSubmit={handleUpdateDecorator} className="space-y-4">
              <input readOnly value={selectedUser.email} className="input input-bordered w-full bg-gray-100 dark:bg-gray-700 dark:text-white dark:border-gray-600 text-sm" />

              <select value={specialty} className="select select-bordered w-full text-sm dark:bg-gray-700 dark:text-white dark:border-gray-600" onChange={(e) => setSpecialty(e.target.value)}>
                <option value="">Select Specialty</option>
                <option value="Wedding">Wedding</option>
                <option value="Corporate">Corporate</option>
                <option value="Birthday">Birthday</option>
              ...
              <input className="input input-bordered w-full text-sm dark:bg-gray-700 dark:text-white dark:border-gray-600" value={phone} onChange={(e) => setPhone(e.target.value)} />
            ...
            <button className="btn btn-outline btn-sm mt-4 sm:mt-6 w-full dark:text-gray-300 dark:border-gray-600" onClick={() => setEditModal(false)}>
              Close
            </button>
          </div>
        </dialog>
