...
//       <title>spark decore | Manage Booking</title>
...
const ManageBookings = () => {
  ...
  return (
    <div className="px-2 sm:px-4 lg:px-6">
      <title>spark decore | Manage Booking</title>
      <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-[#FAB12F] dark:text-amber-400 mt-5 text-center">Manage Bookings</h1>

      {/* Summary Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 sm:gap-5 mb-6">
        <div className="card bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 shadow dark:shadow-gray-900/50">
          <div className="card-body text-center p-3 sm:p-4">
            <h2 className="text-sm sm:text-lg md:text-xl font-bold whitespace-nowrap">Total Bookings</h2>
            <p className="text-xl sm:text-2xl font-bold">{totalBookings}</p>
          </div>
        </div>
        <div className="card bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 shadow dark:shadow-gray-900/50">
          <div className="card-body text-center p-3 sm:p-4">
            <h2 className="text-sm sm:text-lg md:text-xl font-bold whitespace-nowrap">On-site</h2>
            <p className="text-xl sm:text-2xl font-bold">{onSiteCount}</p>
          </div>
        </div>
        <div className="card bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 shadow dark:shadow-gray-900/50">
          <div className="card-body text-center p-3 sm:p-4">
            <h2 className="text-sm sm:text-lg md:text-xl font-bold whitespace-nowrap">In-studio</h2>
            <p className="text-xl sm:text-2xl font-bold">{inStudioCount}</p>
          </div>
        </div>
        <div className="card bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 shadow dark:shadow-gray-900/50">
          <div className="card-body text-center p-3 sm:p-4">
            <h2 className="text-sm sm:text-lg md:text-xl font-bold whitespace-nowrap">Paid</h2>
            <p className="text-xl sm:text-2xl font-bold">{paidCount}</p>
          </div>
      ...
      {/* Search and Sort Section */}
      <div className="rounded-xl shadow-lg dark:shadow-gray-900/50 p-4 sm:p-6 mb-6 bg-[#005461] dark:bg-gray-800">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {/* Search Input */}
          <div className="form-control">
            <label className="label">
            ...
            <div className="relative">
              <input
                type="text"
                placeholder="Search by name, service, location..."
                className="input input-bordered w-full pl-10 focus:outline-none focus:ring-2 focus:ring-[#007b91] dark:focus:ring-teal-400 rounded-lg dark:bg-gray-700 dark:text-white dark:border-gray-600"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
              <FaSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
          ...
          <div className="form-control">
            ...
            <select
              className="select select-bordered w-full focus:outline-none focus:ring-2 focus:ring-[#007b91] dark:focus:ring-teal-400 rounded-lg dark:bg-gray-700 dark:text-white dark:border-gray-600"
              value={dateSort}
              onChange={(e) => setDateSort(e.target.value)}
            >
              <option value="">No Sorting</option>
        ...
        {(searchTerm || dateSort || paymentSort) && (
          <div className="mt-4 text-center">
            <button
              className="bg-[#FAB12F] dark:bg-amber-400 px-3 py-1 rounded-lg text-white dark:text-gray-900 font-bold hover:bg-white hover:text-[#005461] dark:hover:bg-gray-200"
              onClick={() => {
                setSearchTerm("");
                setDateSort("");
                setPaymentSort("");
      ...
      <div className="overflow-x-auto shadow-lg dark:shadow-gray-900/50 rounded-xl bg-white dark:bg-gray-800">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-[#007b91] dark:bg-teal-700 text-white">
            <tr>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm whitespace-nowrap">#</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm whitespace-nowrap">User</th>
              <th className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs sm:text-sm whitespace-nowrap">Service</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {processedBookings.length === 0 ? (
              ...
              processedBookings.map((booking, index) => {
                ...
                return (
                  <tr key={booking._id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    <td className="px-2 sm:px-3 py-2 text-xs sm:text-sm whitespace-nowrap dark:text-gray-300">{index + 1}</td>
                    <td className="px-2 sm:px-3 py-2 text-gray-500 dark:text-gray-400 font-bold text-xs sm:text-sm whitespace-nowrap">{booking.userName}</td>
                    <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                      <span className="bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-300 px-2 py-1 rounded-full font-bold text-xs sm:text-sm">
                        {booking.serviceName}
                      </span>
                    </td>
                    <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                      <span className="bg-sky-200 dark:bg-sky-900/40 text-sky-600 dark:text-sky-300 px-2 py-1 rounded-full font-bold text-xs sm:text-sm">
                        {booking.serviceType}
                      </span>
                    </td>
                    <td className="px-2 sm:px-3 py-2 text-gray-500 dark:text-gray-400 font-bold text-xs sm:text-sm whitespace-nowrap">{booking.bookingDate}</td>
                    <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                      <span className="text-yellow-500 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/40 font-bold px-2 py-1 rounded-full text-xs sm:text-sm">
                        {booking.location}
                      </span>
                    </td>
                    <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                    ...
                    <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                      {booking.decoratorAssigned ? (
                        <span className="text-green-700 dark:text-green-400 font-bold px-2 py-1 text-xs sm:text-sm">
                          {booking.assignedDecoratorName}
                        </span>
                      ) : (
                        <span className="text-red-600 dark:text-red-400 px-2 py-1 font-bold text-xs sm:text-sm">Not Assigned</span>
                      )}
                    </td>
                    <td className="px-2 sm:px-3 py-2 whitespace-nowrap">
                      {isInStudio || !isPaid ? (
                        ...
                        <span className="text-green-700 dark:text-green-400 font-semibold bg-green-200 dark:bg-green-900/40 px-2 py-1 rounded-full text-xs sm:text-sm">Assigned</span>
                      ) : (
                        <button
                          className="btn btn-primary btn-sm rounded-full flex items-center gap-1 text-xs"
                          onClick={() => openDecoratorModal(booking)}
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/40 backdrop-blur-sm transition-opacity p-4 sm:p-6">
          <div className="bg-white dark:bg-gray-800 rounded-3xl w-full sm:w-[600px] max-h-[90vh] overflow-y-auto shadow-2xl dark:shadow-gray-900/50 p-4 sm:p-6 md:p-8 border border-gray-100 dark:border-gray-700 animate-fadeIn">
            {/* Header */}
            <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold mb-4 sm:mb-6 text-center text-gray-900 dark:text-gray-100">
              Select Decorator for <span className="text-[#005461] dark:text-teal-400">{selectedBooking.serviceName}</span>
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
              {decorators.map((d) => (
                <div
                  key={d._id}
                  className="bg-white dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600 shadow-md hover:shadow-lg transition-shadow duration-300 cursor-pointer overflow-hidden relative"
                  onClick={() => assignDecorator(d._id)}
                >
                  {/* Optional: subtle top accent */}
                  <div className="h-1 bg-gradient-to-r from-[#005461] to-[#008080] dark:from-teal-600 dark:to-teal-700 rounded-t-xl"></div>

                  <div className="p-3 sm:p-4 md:p-5">
                    <h3 className="text-base sm:text-lg md:text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">{d.name}</h3>
                    <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-1 truncate">{d.email}</p>
                    <p className="text-xs sm:text-sm italic text-gray-400 dark:text-gray-500">{d.specialty}</p>
                  </div>

                  {/* Hover overlay effect */}
                  <div className="absolute inset-0 bg-white/20 dark:bg-white/10 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-xl"></div>
                </div>
              ))}
            </div>
            ...
            {/* Close Button */}
            <button
              className="mt-4 sm:mt-6 w-full bg-gradient-to-r from-[#005461] to-[#008080] dark:from-teal-600 dark:to-teal-700 hover:from-[#008080] hover:to-[#005461] dark:hover:from-teal-700 dark:hover:to-teal-600 text-white font-bold py-2 sm:py-3 rounded-xl shadow-lg transition-all duration-300 text-sm sm:text-base"
              onClick={() => setShowModal(false)}
            >
              Close
            </button>
